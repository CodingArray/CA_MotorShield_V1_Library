...
void loop() {
  ...
  if (frontDistance < OBSTACLE_THRESHOLD) {
    ...
    delay(500);                  // 대기시간 동안 대기하여 RC카가 회전 후 안정화될 수 있도록 합니다. 이 동안 워치독 타이머를 리셋하여 RC카의 정상 작동을 확인합니다.
  }
}

/**
 * @brief RC카를 뒤로 이동시킨 후 왼쪽으로 회전시키는 함수입니다.
 * 이 함수는 모터 쉴드의 모터 급격한 방향 전환으로부터 큰 부하를 줄여 아두이노 보드와 i2c 통신 안정성을 확보합니다.  
 ...
 * 회전 전후에는 RC카가 부드럽게 멈출 수 있도록 서서히 정지시키는 `softStopAllMotors` 함수를 호출합니다.
 * 또한, `delayWithWdtReset` 함수를 사용하여 지정된 시간만큼 대기하며 워치독 타이머를 리셋합니다.
...
void backwardLeftTurn(unsigned long delayTime) {
  softStopAllMotors();                             // RC카가 부드럽게 멈출 수 있도록 서서히 정지합니다.
  setMotorsDirection(C_BACKWARD, BACKWARD_SPEED);  // RC카를 후진 방향으로 설정하고 후진 속도로 이동을 시작합니다.
  softStopAllMotors();                             // RC카가 부드럽게 멈출 수 있도록 서서히 정지합니다.
  delay(delayTime);                    // 대기시간 동안 대기하여 RC카가 회전 후 안정화될 수 있도록 합니다. 이 동안 워치독 타이머를 리셋하여 RC카의 정상 작동을 확인합니다.
...
void setMotorsDirection(uint8_t direction, uint8_t speed) {
  ...
  softStartAllMotors(direction, speed);  // 모터 속도를 점진적으로 증가시키면서 부드럽게 시작합니다.
}

uint16_t globalFinalSpeed;  // RC카의 모터들이 도달하는 최종 속도를 저장하는 전역 변수입니다.
                            // 이 변수는 모터들의 현재 속도 상태를 기록하고, 속도의 감속 또는 가속에 사용됩니다.

/**
 * @brief RC카의 모든 모터들을 점차적으로 가속시켜 최종 속도까지 도달하게 하는 함수입니다.
 *
 * 이 함수는 globalFinalSpeed가 이미 최종 속도 이상인 경우 즉시 반환하여
 * 불필요한 속도 조절을 방지합니다. 그렇지 않은 경우, 모든 모터의 속도를 MIN_SPEED에서 시작하여
 * 점진적으로 증가시킵니다. 속도는 각 반복마다 5만큼씩 증가하며, 이는 모터가 급격히 가속하는 것을 방지하고
 * 부드러운 가속을 제공합니다. 이 방식은 RC카의 급격한 출발을 방지하고, 더 안정적인 운행을 가능하게 합니다.
 * 
 * 함수는 최종 속도에 도달하면 모든 모터의 속도를 해당 최종 속도로 설정하고,
 * globalFinalSpeed를 최종 속도로 업데이트합니다.
 *
 * @param direction 모터의 방향 (왼쪽, 오른쪽, 전진, 후진 등).
 * @param finalSpeed 모터가 도달할 최종 속도.
 */
void softStartAllMotors(uint8_t direction, uint16_t finalSpeed) {

  // globalFinalSpeed가 이미 최종 속도(finalSpeed) 이상인 경우 함수를 즉시 종료합니다.
  // 이 조건은 이미 목표 속도에 도달했거나 그 이상인 경우 추가적인 속도 증가를 방지하기 위해 사용됩니다.
  if (globalFinalSpeed >= finalSpeed) return;

  uint16_t softStartSpeed = MIN_SPEED;  // 현재 속도를 최소 속도로 시작합니다.

  while (softStartSpeed < finalSpeed) {
    // 모든 모터의 속도를 점진적으로 증가시킵니다.
    motor1->setSpeed(softStartSpeed);
    motor2->setSpeed(softStartSpeed);
    motor3->setSpeed(softStartSpeed);
    motor4->setSpeed(softStartSpeed);

    softStartSpeed += 5;    // 속도를 5만큼 증가시킵니다.
    delay(20);  // 20밀리초 동안 대기합니다.
  }

  // 최종 속도에 도달하면 모든 모터의 속도를 최종 속도로 설정합니다.
  motor1->setSpeed(finalSpeed);
  motor2->setSpeed(finalSpeed);
  motor3->setSpeed(finalSpeed);
  motor4->setSpeed(finalSpeed);

  globalFinalSpeed = finalSpeed;  // 전역 변수에 최종 속도를 저장합니다.
}

/**
 * @brief RC카의 모든 모터들을 점차적으로 감속시켜 정지시키는 함수입니다.
 *
 * 이 함수는 모터들이 현재 속도에서 점차적으로 감속되도록 하여 부드럽게 정지하도록 합니다.
 * 감속 과정에서 모든 모터의 속도를 조절하며, 각 모터의 속도를 점차적으로 감소시킵니다.
 * 이 방법은 RC카의 급격한 정지를 방지하고, 보다 안정적인 정지를 가능하게 합니다.
 *
 * 함수 실행이 끝나면 모든 모터는 정지 상태(RELEASE)가 되며, 
 * 전역 변수 globalFinalSpeed는 최종 속도(감속 후 속도 MIN_SPEED)로 업데이트됩니다.
 */
void softStopAllMotors() {
  uint16_t softStopSpeed = globalFinalSpeed;  // 현재 속도를 전역 변수에서 가져옵니다.

  while (softStopSpeed > MIN_SPEED) {
    // 모든 모터의 속도를 점차 줄여 나갑니다.
    motor1->setSpeed(softStopSpeed);
    motor2->setSpeed(softStopSpeed);
    motor3->setSpeed(softStopSpeed);
    motor4->setSpeed(softStopSpeed);

    softStopSpeed -= 5;     // 속도를 5만큼 감소시킵니다.
    delay(20);  // 20밀리초 동안 대기합니다.
  }

  globalFinalSpeed = MIN_SPEED;  // 전역 변수에 MIN_SPEED 속도를 업데이트합니다.
